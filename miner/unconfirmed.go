...
//unconfirmed是一个数据结构，用来跟踪用户本地的挖矿信息，比如挖出一个块，那么等待足够的后续区块确认之后（5个），再查看本地挖矿的区块是否包含在规范的
//区块链内部
package miner
...
//unconfirmedBlock是本地挖掘区块的一个小的元数据的集合，用来放入未确认的集合，用来追踪本地挖掘的区块是否被包含进入规范的区块
...
//它用来给矿工提供信息，以便他们了解他们之前挖到的区块是否被包含进入来规范的区块链。
...
// report.
//方法shift会删除那些index超过传入的index-depth的区块，并检查他们是否在规范的区块链中
func (set *unconfirmedBlocks) Shift(height uint64) {
	...
	for set.blocks != nil {
		...
		if set.blocks.Value == set.blocks.Next().Value {
			//如果当前的值等于我们自己，说明循环队列只有一个元素，那么设置为nil
			set.blocks = nil
		} else {
